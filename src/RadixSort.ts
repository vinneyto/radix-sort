...
import {
	Fn, If, atomicAdd, globalId, localId, storage, uint, uniform,
...
class RadixSort {
	...
	constructor(
		...
		if ( keys.itemSize !== 1 || indices.itemSize !== 1 || output.itemSize !== 1 ) throw new TypeError( 'keys, indices, and output must have itemSize 1' );
		...
		this.histograms = storage( new StorageBufferAttribute( new Uint32Array( this.maxGroups * RADIX_SIZE ), 1 ), 'uint', this.maxGroups * RADIX_SIZE );
		this.offsets = storage( new StorageBufferAttribute( new Uint32Array( this.maxGroups * RADIX_SIZE ), 1 ), 'uint', this.maxGroups * RADIX_SIZE );
	...
	_buildKernels() {
		...
		this.histogramNodes = this.buffers.map( input => Fn( () => {
			...
			If( position.lessThan( this.count ), () => {
				...
				const digit = this.keys.element( sourceIndex ).shiftRight( this.shift ).bitAnd( RADIX_SIZE - 1 );
		...
		const scatter = ( input: any, output: any ) => Fn( () => {
			...
			If( valid, () => {
				...
				digit.assign( this.keys.element( sourceIndex ).shiftRight( this.shift ).bitAnd( RADIX_SIZE - 1 ) );
			...
			If( valid, () => {
				...
				for ( let previous = 0; previous < WORKGROUP_SIZE; previous ++ ) {
					...
					If( uint( previous ).lessThan( localId.x ).and( tileDigits.element( previous ).equal( digit ) ), () => rank.addAssign( 1 ) );
